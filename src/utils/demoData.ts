...
const RESOURCE_TEMPLATES: Array<{ pattern: string; type: NetworkResource['type'] }> = [
...
export function generateDemoResult(
  ...
  const waterfall: WaterfallEntry[] = resources.slice(0, 15).map((r) => ({
    url: r.url,
    name: r.name,
    type: r.type,
    domain: r.domain,
  ...
  const diagnostics: Diagnostic[] = [
    ...
    { id: 'mainthread-work', title: 'Main thread work', description: 'Time spent on the main thread.', status: 2000 < tbtMs ? 'good' : 5000 < tbtMs ? 'warning' : 'poor', value: Math.round(tbtMs), unit: 'ms' },
  ...
  return {
    ...
    diagnostics,
    javascriptAnalysis: {
      totalSize: resourceBreakdown.javascript.size,
      totalTransferSize: resourceBreakdown.javascript.transferSize,
      scriptCount: jsTotal,
